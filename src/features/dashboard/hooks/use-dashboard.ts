...
export const dashboardKeys = {
  ...
  walletMonthlySummary: (month: string) =>
    [...dashboardKeys.all, "wallet-monthly-summary", month] as const,
}
...
/** month must be a first-of-month date string, e.g. "2026-07-01". */
export function useWalletMonthlySummary(month: string) {
  return useQuery({
    queryKey: dashboardKeys.walletMonthlySummary(month),
    queryFn: () => dashboardRepository.walletMonthlySummary(month),
  })
}
